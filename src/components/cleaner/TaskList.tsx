...
export default function TaskList({ bookings, incidents, properties, loading, onRefresh }: TaskListProps) {
  // STANDARDVY: 'active' (Visar både förfrågningar OCH accepterade städningar blandat efter deadline)
  const [jobFilter, setJobFilter] = useState<'active' | 'finished' | 'all'>('active');
  ...
  // Aktiva städningar (Pending + Accepted blandade och kronologiskt sorterade på deadline)
  ...
  const renderBookingCard = (b: Booking) => {
    ...
    return (
      <div
        key={b.id}
        className={`bg-white text-slate-900 rounded-3xl overflow-hidden shadow-xl transition-all border ${
          isFinished
            ? 'opacity-90 border-emerald-300'
            : isAccepted
            ? 'border-sky-300 shadow-sky-500/10'
            : 'border-amber-400 shadow-amber-500/20 bg-amber-50/20'
        }`}
      >
        <div
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="space-y-2 flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1.5 flex-wrap">
                {isPending && (
                  <span className="text-[10px] font-black px-2.5 py-0.5 rounded-full uppercase tracking-wider bg-amber-400 text-slate-950 border border-amber-500 animate-pulse flex items-center gap-1">
                ...
                {isFinished && (
                  <span className="text-[10px] font-black px-2.5 py-0.5 rounded-full uppercase tracking-wider bg-emerald-600 text-white flex items-center gap-1">
              ...
              {/* DIREKT-KNAPPAR PÅ HOPFÄLLT KORT FÖR SNABB ÅTGÄRD */}
              <div className="pt-1">
                {isPending && (
                  <button
                    type="button"
                    ...
                    className="w-full py-3.5 px-4 bg-emerald-600 hover:bg-emerald-500 text-white font-black text-xs rounded-xl transition shadow-lg shadow-emerald-600/20 flex items-center justify-center gap-2"
                  >
                ...
                {isAccepted && (
                  <button
                    ...
                    disabled={completingId === b.id}
                    className="w-full py-3.5 px-4 bg-sky-600 hover:bg-sky-500 text-white font-black text-xs rounded-xl transition shadow-lg shadow-sky-600/20 flex items-center justify-center gap-2"
                  >
    ...
    <div className="space-y-4">
      ...
      <div className="flex bg-slate-800/80 p-1 rounded-xl text-xs font-bold gap-1 border border-slate-700/60 shadow-md">
        <button
          ...
          className={`flex-1 py-2.5 rounded-lg transition text-center font-black ${
            jobFilter === 'active' ? 'bg-sky-500 text-slate-950 shadow' : 'text-slate-400 hover:text-white'
          }`}
        >
          🧹 Tareas Activas ({activeJobs.length})
        </button>
        <button
          type="button"
        ...
        <button
          ...
          className={`flex-1 py-2.5 rounded-lg transition text-center font-black ${
            jobFilter === 'all' ? 'bg-slate-700 text-white shadow' : 'text-slate-400 hover:text-white'
          }`}
        >
      ...
      {loading ? (
        ...
        /* STANDARDVY: SAMLAD TIDSLINJE SORTERAD PÅ DEADLINE */
        <div className="space-y-3">
          <div className="flex items-center justify-between px-1">
            <span className="text-slate-300 font-black text-xs uppercase tracking-wider flex items-center gap-1.5">
              <Clock className="w-4 h-4 text-sky-400" />
              Próximas limpiezas ({activeJobs.length})
            </span>
            {pendingJobs.length > 0 && (
              <span className="text-[10px] font-black bg-amber-400 text-slate-950 px-2.5 py-0.5 rounded-full animate-pulse">
                {pendingJobs.length} por aceptar
              </span>
            )}
          </div>
          ...
          {activeJobs.length === 0 ? (
            <div className="text-center py-12 bg-slate-800/80 border border-slate-700/80 rounded-3xl p-8 space-y-2">
              <p className="text-white font-black text-lg">Sin tareas pendientes</p>
              <p className="text-slate-400 text-xs">No tienes limpiezas activas programadas en este momento.</p>
            </div>
          ) : (
            activeJobs.map((b) => renderBookingCard(b))
          )}
        </div>
      ) : (
        /* VIST FÖR COMPLETADAS / TODAS */
        <div className="space-y-4">
          {(jobFilter === 'finished' ? finishedJobs : bookings).length === 0 ? (
            <div className="text-center py-12 bg-slate-800/80 border border-slate-700/80 rounded-3xl p-8 space-y-2">
            ...
            (jobFilter === 'finished' ? finishedJobs : bookings)
